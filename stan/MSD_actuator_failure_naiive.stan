...
model {
    ...
    z[1,before+1:N] ~ normal(z[1,before:N-1] + T*z[2,before:N-1], q[1]);
    z[2,before+1:N] ~ normal(z[2,before:N-1] + -(k*T/m)*z[1,before:N-1] + -(b*T/m)*z[2,before:N-1] + (T/m)*(s1*u[1,before:N-1] + s2*u[2,before:N-1]), q[2]); // input affects second state only
