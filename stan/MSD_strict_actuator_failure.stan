...
parameters {
    ...
    // switch parameters
    real<lower=1.0,upper=N> t; // time of actuator failure
    matrix[O,N] z;           // states
}
...
model {
    ...
    z[2,2:before] ~ normal(z[2,1:before-1] + -(k*T/m)*z[1,1:before-1] + -(b*T/m)*z[2,1:before-1] + (T/m)*u[1,1:before-1], q[2]); // input affects second state only
    z_inter[1] = z[1,before] + delt*z[2,before];
    z_inter[2] = z[2,before] + -(k*delt/m)*z[1,before] + -(b*delt/m)*z[2,before] + (delt/m)*u[1,before];
    z[1,before+1] ~ normal(z_inter[1] + (T - delt)*z_inter[2], (delt/T)*q[1] + (T-delt)*q[1]/T);
    z[2,before+1] ~ normal(z_inter[2] + -(k*(T-delt)/m)*z_inter[1] + -(b*(T - delt)/m)*z_inter[2] /*+ ((T - delt)/m)*u[1,before]*/, (delt/T)*q[2] + (T-delt)*q[2]/T); // input affects second state only
    z[1,before+2:N] ~ normal(z[1,before+1:N-1] + T*z[2,before+1:N-1], q[1]);
    z[2,before+2:N] ~ normal(z[2,before+1:N-1] + -(k*T/m)*z[1,before+1:N-1] + -(b*T/m)*z[2,before+1:N-1] /*+ (T/m)*u[1,before+1:N-1]*/, q[2]); // input affects second state only
    
    // measurement likelihood
    y[1,1:before] ~ normal(z[1,1:before], r[1]); // measurement of first state only
    y[2,1:before] ~ normal(-(k/m)*z[1,1:before] - (b/m)*z[2,1:before] + u[1,1:before]/m, r[2]); // acceleration measurement?
    y[1,before+1:N] ~ normal(z[1,before+1:N], r[1]); // measurement of first state only
    y[2,before+1:N] ~ normal(-(k/m)*z[1,before+1:N] - (b/m)*z[2,before+1:N] /*+ u[1,before+1:N]/m*/, r[2]); // acceleration measurement?
}